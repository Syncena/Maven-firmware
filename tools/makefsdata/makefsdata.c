...
#define	HTTPD_ADDITIONAL_CONTENT_TYPES	\
	{"ttf", HTTP_CONTENT_TYPE("text/ttf")}
...
#define	HTTP_HDR_REDIRECT	100
#define	HTTP_HDR_REDIRECT_11	100
...
static void print_usage(void)
{
  ...
  printf("   switch -4: 404 file is treated as a redirect (default is off)" NEWLINE);
                           ...
                           u16_t hdr_len, u16_t hdr_chksum, const u8_t *file_data, size_t file_size)
{
  ...
  fprintf(struct_file, "static const struct fsdata_chksum chksums_%s[] = {" NEWLINE, varname);
                           ...
                           u16_t *http_hdr_chksum, u8_t provide_content_len, int is_compressed)
{
  ...
  if (strstr(filename, "404") == filename) {
    if (redirect404 == 0) {
      ...
      redirect = 1;
      response_type = HTTP_HDR_REDIRECT;
      if (useHttp11) {
        response_type = HTTP_HDR_REDIRECT_11;
      }
    }
  ...
  if (response_type < LWIP_ARRAYSIZE(g_psHTTPHeaderStrings))
    cur_string = g_psHTTPHeaderStrings[response_type];
  else if (response_type == HTTP_HDR_REDIRECT)
    cur_string = "HTTP/1.0 302 Found\r\n";
  else
    cur_string = "HTTP/1.1 302 Found\r\n";
